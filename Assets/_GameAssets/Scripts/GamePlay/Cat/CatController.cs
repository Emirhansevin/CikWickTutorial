...
public class CatController : MonoBehaviour
{
    ...
    private void SetRandomDestinaiton()
    {
        int attempt = 0;
        bool destinaitonSet = false;

        while(attempt < _maxDestinationAttempts && !destinaitonSet)
        {
        ...
        if (!destinaitonSet)
        {
            Debug.Log("Failde to find avalid destination");
            _isWaiting = true;
            _timer = _waitTime * 2;
        }
    }
